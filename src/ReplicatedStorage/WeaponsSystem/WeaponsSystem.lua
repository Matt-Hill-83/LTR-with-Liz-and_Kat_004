...
-- Dependencies
local WeaponsSystemFolder = script.Parent.Parent.WeaponsSystem
print('WeaponsSystemFolder' .. ' - start')
print(WeaponsSystemFolder)
                ...
                coroutine.wrap(
                    function()
                        ...
                        assert(
                            ...
                            string.format(
                                'WeaponType "%s" did not return a valid table',
                                weaponTypeModule:GetFullName()
                            )
                        )
...
function WeaponsSystem.setup()
    ...
    print(WeaponsSystemFolder:GetFullName(), 'is now active.')
    ...
    if IsServer then
        ...
        for _, remoteEventName in pairs(REMOTE_EVENT_NAMES) do
            ...
            WeaponsSystem.connections[remoteEventName .. 'Remote'] =
    ...
    WeaponsSystem.connections.weaponAdded =
        CollectionService:GetInstanceAddedSignal(WEAPON_TAG):Connect(WeaponsSystem.onWeaponAdded)
    WeaponsSystem.connections.weaponRemoved =
...
function WeaponsSystem.onCharacterAdded(character)
    ...
    WeaponsSystem.connections.seated =
        ...
        function()
            ...
            if not weaponType then
                ...
                if weaponTypeObj and weaponTypeObj:IsA('StringValue') then
                    ...
                    warn(
                        'Could not find a WeaponType tag or StringValue for the instance ',
                        weaponInstance:GetFullName()
                    )
                    return
                end
...
-- local module = {runRemoteScript = runRemoteScript}
-- return module
