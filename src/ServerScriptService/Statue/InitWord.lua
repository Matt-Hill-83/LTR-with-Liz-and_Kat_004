...
local function configWord(props)
    ...
    local imageFound = Utils.applyDecalsToCharacterFromWord({part = newWord, word = word})
    ...
    wordPositioner.CFrame =
        Utils3.setCFrameFromDesiredEdgeOffset(
        {
            ...
            offsetConfig = {
                ...
                offsetAdder = Vector3.new((offsetX + currentWordPosition.value), 0, 0)
            }
        }
    )
...
local function initWord(props)
    ...
    local wordBenchSizeX = (#word - 1) * totalLetterWidth + letterBlockTemplate.Size.X
