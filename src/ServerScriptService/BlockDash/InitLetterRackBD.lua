...
local function initLetterRack(miniGameState)
    ...
    for beltPlateIndex, beltPlate in ipairs(beltPlates) do
        ...
        local letterPositioner = Utils.getFirstDescendantByName(beltPlate, 'LetterPositioner')
        ...
        for colIndex = 1, numCol do
            for rowIndex = 1, numRow do
                ...
                newLetterBlock.Size = Vector3.new(rackLetterSize, rackLetterSize, rackLetterSize)
                ...
                local letterPosX = -newLetterBlock.Size.X * (colIndex - 1) * spacingFactorX
                local letterPosZ = -newLetterBlock.Size.Z * (rowIndex - 1) * spacingFactorZ
                ...
                local function onTouchBlock(newLetterBlock, miniGameState)
                    ...
                    local function closure(otherPart)
                        ...
                        if not db.value then
                            ...
                            miniGameState.onSelectRackBlock(newLetterBlock, miniGameState, player)
                            db.value = false
                        end
                    end
                    return closure
                ...
                newLetterBlock.Touched:Connect(onTouchBlock(newLetterBlock, miniGameState))
