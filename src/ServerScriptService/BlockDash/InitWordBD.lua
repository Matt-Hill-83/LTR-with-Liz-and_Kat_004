...
local function initWord(props)
    ...
    local totalWidth = totalLetterWidth * maxWordLength * math.ceil(#words / wordsPerCol) - wordLetterSize / 2

    local colOffsetX = (colIndex - 1) * (totalLetterWidth * maxWordLength + wordOffset) - totalWidth / 2
    ...
    wordBench.CFrame =
        Utils3.setCFrameFromDesiredEdgeOffset(
        {
            ...
            offsetConfig = {
                ...
                offsetAdder = Vector3.new(0, offsetY * (wordIndex - 1), colOffsetX)
            }
        }
    )
    ...
    for letterIndex = 1, #word do
        ...
        newLetter.Size = Vector3.new(wordLetterSize, wordLetterSize, wordLetterSize)

        local offsetX = -(newLetter.Size.X * (letterIndex - 1) * spacingFactorX) - 0
