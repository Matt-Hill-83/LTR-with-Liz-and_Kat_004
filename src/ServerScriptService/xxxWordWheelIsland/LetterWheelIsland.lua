...
function configCouchTrigger(miniGameState)
    ...
    for i, seat in ipairs(seats) do
        ...
        function initGame()
            local cameraPath1 = Utils.getFirstDescendantByName(letterFallFolder, 'ScreenCameraPath1')
            local cameraPath2 = Utils.getFirstDescendantByName(letterFallFolder, 'ScreenCameraPath2')
            ...
            if humanoid then
                ...
                if player then
                    ...
                    letterFallFreezeCameraRE:FireClient(player, cameraPath1, cameraPath2, true)
                    return
                end
            end
            ...
            if currentPlayer then
                letterFallFreezeCameraRE:FireClient(currentPlayer, cameraPath1, cameraPath2, false)
                currentPlayer = nil
            end
        end
        ...
        seat:GetPropertyChangedSignal('Occupant'):Connect(initGameWrapper(miniGameState))
    end
end

function initGameToggle(miniGameState)
    local letterFallFolder = miniGameState.letterFallFolder
    local startGameTrigger = Utils.getFirstDescendantByName(letterFallFolder, 'StartGameTrigger')

    function onCorrectItemDropped()
        local manHoleCover = Utils.getFirstDescendantByName(letterFallFolder, 'ManHoleCover')
        if manHoleCover then
            manHoleCover:Destroy()
        end
    end

    local dropBox = Utils.getFirstDescendantByName(letterFallFolder, 'DropBoxModel')
